...
const AddProduct = ({ productToEdit = {}, isEditing = false }) => {
  ...
  const onSubmit = (data) => {
    const selectedColors = COLORS.filter((color) => data.colors.includes(color));

    if (isEditing) {
      update({
        id: productToEdit.id,
        product: { ...data, colors: selectedColors },
      });
    } else {
      addProduct({ ...data, colors: selectedColors });
    }
  };
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {COLORS.map((color) => (
                    <label key={color} className="flex items-center space-x-2 text-black">
                      <input
                        type="checkbox"
                        value={color}
                        {...register("colors")}
                        defaultChecked={productToEdit.colors?.includes(color)}
                      />
